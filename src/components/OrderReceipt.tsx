...
const OrderReceipt = ({
  ...
  const handleDownload = () => {
    ...
    if (isMobile) {
      // Mobile : ouvrir le reçu dans le MÊME onglet via Blob URL.
      // Aucune popup, aucune autorisation à donner. L'utilisateur peut
      // ensuite utiliser "Partager → Imprimer / Enregistrer en PDF"
    ...
    if (!printWindow) {
      // Fallback si popups bloqués : ouvrir dans le même onglet
      const blob = new Blob([content], { type: "text/html" });
      const url = URL.createObjectURL(blob);
      window.location.href = url;
      return;
